...
async function changeVariant(oldVariant) {
  let newVariant = new HTMLRewriter()
    .on('h1#title', {
      element(element) {
        element.setInnerContent(`The Awesome Variant`)
      },
    })
    .on('title', {
      element(element) {
        element.setInnerContent(`Totally Awesome`)
      }
    })
    .on('p#description', {
      element(element) {
        element.setInnerContent(`Hello World, I'm Kamal Walia`)
      }
    })
    .on('a', {
      element(element) {
        element.setAttribute("href", "https://www.linkedin.com/in/kamal1709/")
        element.setInnerContent(`Connect with me on my LinkedIn`)
      }
    })
